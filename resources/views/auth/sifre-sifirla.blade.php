@extends('layouts.app')
@section('title', ceviri('Yeni Şifre Belirle') . ' — SUÇEK')
...
<div class="min-h-[calc(100vh-80px)] grid grid-cols-1 lg:grid-cols-5">
  ...
  <div class="hidden lg:flex lg:col-span-2 bg-[#0F0F0F] flex-col justify-between px-10 py-14 relative overflow-hidden">
    ...
    <div class="relative z-10 space-y-8">
      <div>
        <p class="font-serif-sc text-[28px] text-white leading-[1.3]">{!! ceviri('Yeni şifrenizi <em>belirleyin.</em>') !!}</p>
        <p class="text-[13px] text-[rgba(255,255,255,0.45)] mt-3 leading-relaxed">
          {{ ceviri('Güçlü bir şifre seçin. En az 8 karakter uzunluğunda olmalıdır.') }}
        </p>
      </div>
    </div>
  ...
  <div class="lg:col-span-3 flex items-center justify-center px-6 py-14 bg-[#FAFAFA]">
    <div class="w-full max-w-md">
      ...
      <div class="mb-8">
        <h1 class="font-display text-[28px] font-semibold text-[#0F0F0F]">{{ ceviri('Yeni Şifre Belirle') }}</h1>
        <p class="text-[13px] text-[#A8A8A8] mt-1">{{ ceviri('Hesabınız için yeni bir şifre oluşturun.') }}</p>
      </div>
      ...
      <form action="{{ route('sifre-sifirla.kaydet') }}" method="POST" class="space-y-5"
        ...
        <div>
          <label for="password" class="block text-[11px] font-semibold text-[#5A5A5A] uppercase tracking-[.08em] mb-2">{{ ceviri('Yeni Şifre') }}</label>
          <div class="relative">
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button type="button" @click="showPass2 = !showPass2"
                    class="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#A8A8A8] hover:text-[#0F0F0F] transition-colors p-1 cursor-pointer"
                    :aria-label="showPass2 ? '{{ ceviri('Şifreyi gizle') }}' : '{{ ceviri('Şifreyi göster') }}'">
        ...
        <button type="submit"
          ...
          {{ ceviri('Şifremi Sıfırla') }}
        </button>
      </form>

      <div class="mt-8 pt-6 border-t border-[rgba(0,0,0,0.07)] text-center">
        <p class="text-[13px] text-[#A8A8A8]">
          <a href="{{ route('sifremi-unuttum') }}" class="text-[#0F0F0F] font-semibold hover:underline">← {{ ceviri('Tekrar sıfırlama bağlantısı iste') }}</a>
        </p>
      </div>
