@extends('layouts.app')
@section('title', ceviri('Şifremi Unuttum') . ' — SUÇEK')
...
<div class="min-h-[calc(100vh-80px)] grid grid-cols-1 lg:grid-cols-5">
  ...
  <div class="hidden lg:flex lg:col-span-2 bg-[#0F0F0F] flex-col justify-between px-10 py-14 relative overflow-hidden">
    ...
    <div class="relative z-10 space-y-8">
      <div>
        <p class="font-serif-sc text-[28px] text-white leading-[1.3]">{!! ceviri('Şifrenizi <em>güvenle</em> sıfırlayın.') !!}</p>
        <p class="text-[13px] text-[rgba(255,255,255,0.45)] mt-3 leading-relaxed">
          {{ ceviri('E-posta adresinize sıfırlama bağlantısı göndereceğiz. Birkaç dakika içinde yeni şifrenizi belirleyebilirsiniz.') }}
        </p>
      </div>
    </div>
  ...
  <div class="lg:col-span-3 flex items-center justify-center px-6 py-14 bg-[#FAFAFA]">
    <div class="w-full max-w-md">
      ...
      <div class="mb-8">
        <h1 class="font-display text-[28px] font-semibold text-[#0F0F0F]">{{ ceviri('Şifremi Unuttum') }}</h1>
        <p class="text-[13px] text-[#A8A8A8] mt-1">{{ ceviri('Kayıtlı e-posta adresinizi girin, sıfırlama bağlantısı gönderelim.') }}</p>
      </div>
      ...
      <div class="mb-5 flex items-start gap-3 bg-[#E6F4EC] border border-[#9DD4B5] text-[#1A5C3A] rounded-[10px] px-4 py-3.5 text-[13px]" role="alert">
        ...
        <div>
          <p class="font-semibold mb-0.5">{{ ceviri('E-posta gönderildi') }}</p>
          <p>{{ session('basari') }}</p>
        </div>
      </div>
      @else
      ...
      <div class="mt-8 pt-6 border-t border-[rgba(0,0,0,0.07)] text-center space-y-2">
        <p class="text-[13px] text-[#A8A8A8]">
          {{ ceviri('Şifrenizi hatırladınız mı?') }}
