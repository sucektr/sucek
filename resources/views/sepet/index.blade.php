...
@section('title', ceviri('Sepetim') . ' — SUÇEK')

@section('content')

<section class="section min-h-[50vh]" aria-label="{{ ceviri('Alışveriş sepeti') }}">
  <div class="mb-6">
    <p class="text-[9px] font-medium tracking-[3px] uppercase text-[#A8A8A8] mb-1.5">{{ ceviri('ALIŞVERİŞ') }}</p>
    <h1 class="font-serif-sc text-[28px] font-bold text-[#0F0F0F]">{{ ceviri('Sepetim') }}</h1>
  </div>
  ...
  <div class="flex flex-col items-center justify-center py-20 text-center bg-[#F9F9F9] rounded-[16px]">
    ...
    <p class="text-[14px] font-medium text-[#5A5A5A] mb-1">{{ ceviri('Sepetiniz boş') }}</p>
    <p class="text-[12px] text-[#A8A8A8] mb-6">{{ ceviri('Ürünlerimizi keşfetmek için mağazamıza göz atın.') }}</p>
    <div class="flex gap-3">
      <a href="{{ route('magaza.index') }}" class="btn btn-dark text-[10px] tracking-[1.5px] px-6 py-2.5 min-h-[44px]">{{ ceviri('Mağazaya Git') }}</a>
      <a href="{{ route('koleksiyon.index') }}" class="btn btn-outline text-[10px] tracking-[1.5px] px-6 py-2.5 min-h-[44px]">{{ ceviri('Koleksiyon') }}</a>
    </div>
  </div>
  @else
  ...
  <div class="lg:hidden mb-4" x-data="cartData()">
    <a href="{{ route('siparis.checkout') }}"
      ...
      <i class="ti ti-lock text-sm mr-1.5" aria-hidden="true"></i>{{ ceviri('Siparişi Tamamla') }}
    </a>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-data="cartData()">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-2">
              ...
              <form action="{{ route('sepet.kaldir', $key) }}" method="POST"
                    onsubmit="return confirm('{{ ceviri('Bu ürünü sepetten kaldırmak istediğinize emin misiniz?') }}')"
                    class="shrink-0">
                @csrf @method('DELETE')
                <button type="submit"
                        class="w-7 h-7 flex items-center justify-center rounded-[6px] text-[#A8A8A8] hover:text-red-500 hover:bg-red-50 transition-all"
                        aria-label="{{ $item['ad'] }} {{ ceviri('sepetten kaldır') }}">
            ...
            <p class="flex items-center gap-1 text-[11px] text-[#A8A8A8] mt-0.5">
              ...
              {{ number_format($item['kargo_ucreti'], 2, ',', '.') }} ₺ {{ ceviri('kargo') }}
            </p>
            @else
            <p class="flex items-center gap-1 text-[11px] text-green-600 mt-0.5">
              <i class="ti ti-truck text-[11px]" aria-hidden="true"></i>
              {{ ceviri('Ücretsiz kargo') }}
            </p>
            @endif
    ...
    <div class="lg:col-span-1">
      <div class="bg-white border border-[rgba(0,0,0,0.07)] rounded-[16px] p-6 sticky top-24">
        <h2 class="font-display text-[20px] font-semibold text-[#0F0F0F] mb-5 pb-4 border-b border-[rgba(0,0,0,0.08)]">{{ ceviri('Sipariş Özeti') }}</h2>
        <dl class="space-y-3 mb-5">

          <div class="flex justify-between text-[13px]">
            <dt class="text-[#5A5A5A]">{{ ceviri('Ara Toplam (KDV hariç)') }}</dt>
          ...
          <div class="flex justify-between text-[13px]" x-show="kdvToplam > 0" x-cloak>
            <dt class="text-[#5A5A5A]">{{ ceviri('KDV') }}</dt>
            <dd class="font-medium" x-text="fmt(kdvToplam) + ' ₺'">{{ number_format($kdvToplam, 2, ',', '.') }} ₺</dd>
          </div>

          <div class="flex justify-between text-[13px]">
            <dt class="text-[#5A5A5A]">{{ ceviri('Kargo') }}</dt>
            <dd class="font-medium"
                :class="kargoUcreti > 0 ? 'text-[#0F0F0F]' : 'text-green-600'"
                x-text="kargoUcreti > 0 ? fmt(kargoUcreti) + ' ₺' : '{{ ceviri('Ücretsiz') }}'">
              @if($kargoUcreti > 0){{ number_format($kargoUcreti, 2, ',', '.') }} ₺@else<span class="text-green-600">{{ ceviri('Ücretsiz') }}</span>@endif
            </dd>
          </div>
          ...
          <div class="flex items-center gap-1 text-[11px] text-[#A8A8A8] -mt-1" x-show="eksik > 0" x-cloak>
            ...
            <span x-text="fmt(eksik) + ' ₺ {{ ceviri('daha ekleyerek ücretsiz kargoya ulaşın') }}'"></span>
          </div>

          <div class="flex justify-between pt-3 border-t border-[rgba(0,0,0,0.08)]">
            <dt class="font-semibold text-[14px]">{{ ceviri('Toplam') }}</dt>
