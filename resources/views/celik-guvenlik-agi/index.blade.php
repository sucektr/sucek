...
@section('title', ceviri('Çelik Güvenlik Ağı') . ' — MESH ' . ceviri('Yetkili Bayii') . ' | SUÇEK')
@section('meta-description', ceviri('SUÇEK, MESH Çelik Ağ yetkili bayii olarak inşaat güvenliğinden balkon korumaya kadar her ihtiyaca özel çelik güvenlik ağı çözümleri sunmaktadır.'))
...
<section class="bg-[#F8FAFC] border-b border-[#E2E8F0]">
  <div class="max-w-[1280px] mx-auto px-4 lg:px-6 py-12 lg:py-16">
    ...
    <div class="text-center mb-10">
      <p class="text-[10px] font-semibold tracking-widest uppercase text-[#CC2200] mb-2">{{ ceviri('UYGULAMA ALANLARI') }}</p>
      <h2 class="text-[24px] lg:text-[30px] font-bold text-[#0F172A] tracking-tight">{{ ceviri('Nerelerde Kullanılır?') }}</h2>
    </div>
...
<section class="bg-white border-b border-[#E2E8F0]">
  <div class="max-w-[1280px] mx-auto px-4 lg:px-6 py-12 lg:py-14">
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      @foreach([
        ['icon'=>'ti-shield-check',  'baslik'=>ceviri('Yüksek Güvenlik'),   'metin'=>ceviri('EN ISO 1263 standartları')],
        ['icon'=>'ti-ruler-measure', 'baslik'=>ceviri('Özel Ölçü'),         'metin'=>ceviri('Her projeye uygun boyut')],
        ['icon'=>'ti-certificate-2', 'baslik'=>ceviri('Sertifikalı Ürün'),  'metin'=>ceviri('TSE belgeli üretim')],
        ['icon'=>'ti-tools',         'baslik'=>ceviri('Teknik Destek'),     'metin'=>ceviri('Montaj & uygulama rehberliği')],
      ] as $o)
...
<section id="urunler" class="bg-white border-b border-[#E2E8F0]">
  <div class="max-w-[1280px] mx-auto px-4 lg:px-6 py-12 lg:py-16">
    ...
    <div class="flex items-end justify-between mb-8">
      <div>
        <p class="text-[10px] font-semibold tracking-widest uppercase text-[#CC2200] mb-2">{{ ceviri('ÜRÜN KATALOĞU') }}</p>
        <h2 class="text-[22px] lg:text-[28px] font-bold text-[#0F172A] tracking-tight">{{ ceviri('Güvenlik Ağı Ürünleri') }}</h2>
      </div>
      @if($urunler->count() > 0)
      <span class="text-[13px] text-[#94A3B8]">{{ $urunler->count() }} {{ ceviri('ürün') }}</span>
      @endif
    </div>
    ...
    <div class="text-center py-16 bg-[#F8FAFC] rounded-2xl border border-[#E2E8F0]">
      ...
      <h3 class="text-[16px] font-bold text-[#0F172A] mb-2">{{ ceviri('Ürünler Yakında') }}</h3>
      <p class="text-[13px] text-[#64748B] max-w-sm mx-auto mb-6">{{ ceviri('Çelik güvenlik ağı ürün kataloğumuz hazırlanmaktadır. Fiyat ve bilgi almak için bizimle iletişime geçin.') }}</p>
      <a href="{{ route('iletisim.index') }}"
