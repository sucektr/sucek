...
<section class="section">
  ...
  {{-- Tüm filtreleme ve grid tek x-data scope içinde --}}
  <div x-data="{ aktif: 'tumu' }">
    ...
    <div class="flex flex-wrap gap-2 mb-8">
      ...
      <button @click="aktif = '{{ $kat }}'"
              ...
              class="px-4 py-2 rounded-[8px] border text-[12px] font-semibold tracking-[0.5px] transition-all duration-150 cursor-pointer">
        {{ $katLabel }} <span class="ml-1 opacity-60">({{ $projeler->where('kategori', $kat)->count() }})</span>
      </button>
      @endforeach
    </div>
    @endif
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      <article
        ...
        @endif
        class="group bg-white rounded-xl border border-[#E2E8F0] overflow-hidden hover:shadow-[0_8px_32px_rgba(15,23,42,0.10)] hover:-translate-y-1 transition-all duration-300">
        ...
        <a href="{{ route('projeler.show', $proje->slug) }}" class="block">
          ...
          <div class="p-5">
            ...
            @if($proje->konum)
            <div class="flex items-center gap-1.5 text-[12px] text-[#94A3B8] mt-2">
