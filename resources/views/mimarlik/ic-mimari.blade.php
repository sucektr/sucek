...
@section('title', ceviri('İç Mimari') . ' — SUÇEK Mimarlık')
...
{{-- ─── Hero ──────────────────────────────────────────────────────────── --}}
<section class="relative overflow-hidden min-h-[320px] flex items-end" aria-label="{{ ceviri('İç Mimari hero') }}">
...
<section class="section {{ icerik('icmimari','icerik','') ? 'border-t border-[rgba(0,0,0,0.06)]' : '' }}" aria-labelledby="icmimari-projeler-baslik">
  <div class="flex items-end justify-between mb-6">
    <div>
      ...
      <h2 id="icmimari-projeler-baslik" class="font-serif-sc text-[28px] font-bold text-[#0F0F0F]">{{ ceviri('Tamamlanan Projeler') }}</h2>
    </div>
...
<section class="section">
  <div class="bg-[#141414] rounded-[16px] px-10 lg:px-16 py-12 flex flex-col lg:flex-row items-center justify-between gap-6">
    <div>
      <h2 class="font-display text-[28px] lg:text-[34px] font-semibold text-white mb-2">{{ ceviri('Projenizi Birlikte Tasarlayalım') }}</h2>
