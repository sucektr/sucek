...
{{-- ─── İletişim ──────────────────────────────────────────────────────── --}}
<section class="section" id="iletisim" aria-labelledby="iletisim-baslik">
  <div class="mb-7">
    <p class="section-label mb-2">BİZE ULAŞIN</p>
    <h2 id="iletisim-baslik" class="text-[26px] font-bold text-[#0F172A] tracking-tight">İletişim</h2>
  </div>

  @if(session('basari'))
  <div class="mb-5 flex items-center gap-3 bg-green-50 border border-green-200 text-green-800 text-[13px] px-5 py-4 rounded-xl" role="alert">
    <i class="ti ti-circle-check text-xl text-green-500" aria-hidden="true"></i>
    {{ session('basari') }}
  </div>
  @endif
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    ...
    <div class="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-6 flex flex-col gap-5">
      <h3 class="text-[15px] font-bold text-[#0F172A] border-b border-[#E2E8F0] pb-4">İletişim Bilgileri</h3>
      <ul class="space-y-4 flex-1">
        <li class="flex items-start gap-3">
          <div class="w-8 h-8 rounded-lg bg-white border border-[#E2E8F0] flex items-center justify-center shrink-0 mt-0.5">
            <i class="ti ti-map-pin text-sm text-[#64748B]" aria-hidden="true"></i>
          </div>
          <span class="text-[13px] text-[#64748B] leading-relaxed">{{ icerik('site','adres','Etimesgut, Ankara') }}</span>
        </li>
        <li class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-lg bg-white border border-[#E2E8F0] flex items-center justify-center shrink-0">
            <i class="ti ti-phone text-sm text-[#64748B]" aria-hidden="true"></i>
          </div>
          <a href="tel:{{ preg_replace('/[^+\d]/', '', icerik('site','telefon','+905442948402')) }}"
             class="text-[13px] text-[#64748B] hover:text-[#0F172A] transition-colors">
            {{ icerik('site','telefon','+90 (544) 294 84 02') }}
          </a>
        </li>
        <li class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-lg bg-white border border-[#E2E8F0] flex items-center justify-center shrink-0">
            <i class="ti ti-mail text-sm text-[#64748B]" aria-hidden="true"></i>
          </div>
      ...
      <div class="border-t border-[#E2E8F0] pt-4">
        <p class="text-[11px] font-semibold tracking-wider uppercase text-[#94A3B8] mb-2">ÇALIŞMA SAATLERİ</p>
        <p class="text-[13px] text-[#64748B]">{{ icerik('site','calisma_hafta','Pzt–Cum: 09:00–18:00') }}</p>
        <p class="text-[13px] text-[#64748B]">{{ icerik('site','calisma_cumartesi','Cumartesi: 10:00–15:00') }}</p>
      </div>
    </div>
    ...
    <form id="anasayfa-iletisim-form" action="{{ route('iletisim.gonder') }}" method="POST" class="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm flex flex-col" novalidate>
      ...
      <div class="flex flex-col gap-4 flex-1">
        <div>
          <label for="c-ad" class="block text-sm font-medium text-[#64748B] mb-1.5">
            Adınız <span class="text-[#CC2200]">*</span>
          </label>
          <input id="c-ad" type="text" name="ad" required autocomplete="name"
                 value="{{ old('ad') }}" placeholder="Ad Soyad"
                 class="w-full bg-white border border-[#E2E8F0] rounded-lg px-4 py-2.5 text-[14px] text-[#0F172A] placeholder:text-[#CBD5E1] outline-none focus:border-[#CC2200] focus:ring-2 focus:ring-[rgba(204,34,0,0.08)] transition-all min-h-[44px]">
          @error('ad')<p class="text-[11px] text-red-500 mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
          <label for="c-email" class="block text-sm font-medium text-[#64748B] mb-1.5">
            E-posta <span class="text-[#CC2200]">*</span>
          </label>
        ...
        <div>
          <label for="c-mesaj" class="block text-sm font-medium text-[#64748B] mb-1.5">
            Mesajınız <span class="text-[#CC2200]">*</span>
          </label>
          <textarea id="c-mesaj" name="mesaj" required rows="4" placeholder="Mesajınızı yazın..."
                    class="w-full bg-white border border-[#E2E8F0] rounded-lg px-4 py-2.5 text-[14px] text-[#0F172A] placeholder:text-[#CBD5E1] outline-none focus:border-[#CC2200] focus:ring-2 focus:ring-[rgba(204,34,0,0.08)] transition-all resize-none">{{ old('mesaj') }}</textarea>
