...
@section('title', ceviri('Ödeme Başarılı') . ' — SUÇEK')
...
<div class="section max-w-lg mx-auto text-center py-10">
  ...
  <h1 class="font-serif-sc text-[28px] font-bold text-[#0F0F0F] mb-2">{{ ceviri('Ödemeniz Alındı!') }}</h1>
  ...
  <p class="text-[13px] text-[#5A5A5A] leading-relaxed mb-8 max-w-sm mx-auto">
    {{ ceviri('Siparişiniz hazırlanmaya başlanacaktır. Kargo bilgileri e-posta adresinize') }} (<strong>{{ $siparis->email }}</strong>) {{ ceviri('iletilecektir.') }}
  </p>
  ...
  <div class="flex flex-col sm:flex-row gap-3 justify-center">
    ...
    <a href="{{ route('hesabim.index') }}"
      ...
      <i class="ti ti-package text-sm"></i>{{ ceviri('Siparişlerim') }}
    </a>
    @endauth
    <a href="{{ route('magaza.index') }}"
      ...
      <i class="ti ti-shopping-bag text-sm"></i>{{ ceviri('Alışverişe Devam') }}
    </a>
  </div>
