...
@section('title', ceviri('Güvenli Ödeme') . ' — SUÇEK')

@section('content')
<div class="section max-w-2xl mx-auto">

  <div class="mb-6 text-center">
    <p class="text-[9px] font-medium tracking-[3px] uppercase text-[#A8A8A8] mb-1.5">{{ ceviri('ÖDEME') }}</p>
    <h1 class="font-serif-sc text-[26px] font-bold text-[#0F0F0F]">{{ ceviri('Güvenli Ödeme') }}</h1>
    <p class="text-[11px] text-[#A8A8A8] mt-1">{{ ceviri('Sipariş:') }} <span class="font-medium text-[#5A5A5A]">#{{ $siparis->referans }}</span></p>
  </div>
  ...
  <div class="bg-white rounded-[12px] border border-[rgba(0,0,0,0.07)] overflow-hidden shadow-sm">
    ...
    <div class="px-5 py-4 border-b border-[rgba(0,0,0,0.06)] flex items-center justify-between bg-[#FAFAFA]">
      <div class="flex items-center gap-2">
        ...
        <span class="text-[11px] font-semibold text-[#0F0F0F] tracking-[0.5px]">{{ ceviri('SSL Güvenli Ödeme') }}</span>
      </div>
