...
@section('title', ceviri('İnşaat') . ' — SUÇEK')
...
{{-- ─── Hero ──────────────────────────────────────────────────────────── --}}
<section class="relative overflow-hidden min-h-[360px] flex" aria-label="{{ ceviri('İnşaat hero') }}">
  ...
  <div class="relative z-10 flex flex-col lg:flex-row items-end lg:items-center gap-8 px-5 lg:px-14 py-12 lg:py-20 w-full">
    ...
    <div class="flex flex-col sm:flex-row lg:flex-col gap-2 shrink-0">
      <a href="{{ route('insaat.hesaplama') }}"
        ...
        <i class="ti ti-calculator text-sm" aria-hidden="true"></i> {{ ceviri('Maliyet Hesapla') }}
      </a>
      <a href="{{ route('home') }}#iletisim"
         class="btn btn-outline-inv text-[10px] tracking-[1.5px] px-6 py-3 min-h-[44px] justify-center">
        <i class="ti ti-calendar text-sm" aria-hidden="true"></i> {{ ceviri('Teklif Al') }}
      </a>
    </div>
  </div>
</section>
...
<section class="section" id="hizmetler" aria-labelledby="ins-hizmetler-baslik">
  <div class="mb-6">
    <p class="text-[9px] font-medium tracking-[3px] uppercase text-[#A8A8A8] mb-1.5">{{ ceviri('HİZMET PAKETLERİ') }}</p>
    <h2 id="ins-hizmetler-baslik" class="font-serif-sc text-[28px] font-bold text-[#0F0F0F]">{{ ceviri('İnşaat Hizmetlerimiz') }}</h2>
  </div>
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
    ...
    <article class="group bg-white border border-[rgba(0,0,0,0.07)] rounded-[12px] p-6 hover:shadow-[0_4px_20px_rgba(0,0,0,0.10)] hover:-translate-y-0.5 transition-all duration-200 relative">
      @if($h['one_cikan'])
      <span class="absolute top-4 right-4 text-[8px] font-bold tracking-[1.5px] uppercase bg-[#141414] text-white px-2 py-1 rounded-[4px]">{{ ceviri('Popüler') }}</span>
      @endif
...
<section class="section" aria-labelledby="ins-portfolio-baslik">
  <div class="mb-6">
    <p class="text-[9px] font-medium tracking-[3px] uppercase text-[#A8A8A8] mb-1.5">{{ ceviri('SON ÇALIŞMALAR') }}</p>
    <h2 id="ins-portfolio-baslik" class="font-serif-sc text-[28px] font-bold text-[#0F0F0F]">{{ ceviri('Tamamlanan Projeler') }}</h2>
  </div>
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
...
<section class="section">
  <div class="bg-[#141414] rounded-[16px] px-6 lg:px-16 py-10 lg:py-12 flex flex-col lg:flex-row items-center justify-between gap-6 text-center lg:text-left">
    ...
    <a href="{{ route('insaat.hesaplama') }}"
      ...
      <i class="ti ti-calculator text-sm mr-1" aria-hidden="true"></i> {{ ceviri('Hesaplama Aracı') }}
    </a>
  </div>
</section>
...
<section class="section pt-0">
  <div class="bg-[#141414] rounded-[16px] px-6 lg:px-16 py-10 lg:py-12 flex flex-col lg:flex-row items-center justify-between gap-6 text-center lg:text-left">
    ...
    <a href="{{ route('insaat.emsal') }}"
       class="btn bg-white text-[#0F0F0F] text-[10px] tracking-[1.5px] px-8 py-3.5 min-h-[44px] justify-center hover:bg-[#F0F0F0] transition-colors rounded-[8px] shrink-0">
      <i class="ti ti-ruler-measure text-sm mr-1" aria-hidden="true"></i> {{ ceviri('Emsal Hesapla') }}
    </a>
  </div>
</section>
