...
<form action="{{ route('admin.icerik.guncelle', $sayfa) }}" method="POST" enctype="multipart/form-data">
  ...
  <div class="flex gap-1 mb-5">
    <button type="button" id="sekme-tr" onclick="dilSec('tr')"
            class="flex items-center gap-1.5 px-4 py-2 rounded-[8px] text-[12px] font-semibold border border-[#CC2200] bg-[#CC2200] text-white transition-colors">
      <span>🇹🇷</span> Türkçe
    </button>
    <button type="button" id="sekme-en" onclick="dilSec('en')"
            class="flex items-center gap-1.5 px-4 py-2 rounded-[8px] text-[12px] font-semibold border border-[#E2E8F0] bg-white text-[#64748B] hover:border-[#CC2200] hover:text-[#CC2200] transition-colors">
      <span>🇬🇧</span> English
    </button>
  </div>

  @foreach(['tr' => '', 'en' => '_en'] as $aktifDil => $suffix)
  <div id="panel-{{ $aktifDil }}" class="space-y-4 max-w-3xl" @if($aktifDil === 'en') style="display:none" @endif>
    @foreach($tanim['alanlar'] as $alan_tanim)
    @php
      $alan   = $alan_tanim['alan'];
      $tip    = $alan_tanim['tip'];
      $rowKey = $aktifDil === 'tr' ? $alan : $alan . '_en_row';
      $row    = $mevcut[$rowKey] ?? null;
      $uid    = $alan . $suffix;
    @endphp
    ...
    <div class="bg-white rounded-[12px] border border-[#E2E8F0] p-6">
      ...
      @if($tip === 'gorsel')
        @if($row && $row->gorsel)
        <div class="mb-3 flex items-start gap-3" id="gorsel-wrap-{{ $uid }}">
          ...
          <div class="flex flex-col gap-2 pt-1">
            <input type="hidden" name="f_{{ $uid }}_sil" value="0" id="sil-flag-{{ $uid }}">
            <button type="button"
                    onclick="document.getElementById('sil-flag-{{ $uid }}').value='1';document.getElementById('gorsel-wrap-{{ $uid }}').style.display='none';document.getElementById('gorsel-yeni-{{ $uid }}').style.display='flex';document.getElementById('gorsel-info-{{ $uid }}').style.display='none';"
        ...
        <p class="text-[11px] text-[#94A3B8] mb-2" id="gorsel-info-{{ $uid }}">Yeni görsel yükleyerek değiştirebilirsiniz:</p>
        @endif
        @error('f_' . $uid)
        ...
        @enderror
        <div id="gorsel-yeni-{{ $uid }}" style="{{ ($row && $row->gorsel) ? 'display:none' : '' }}">
        <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed {{ $errors->has('f_'.$uid) ? 'border-red-400 bg-red-50' : 'border-[#E2E8F0] bg-[#F8FAFC]' }} rounded-[8px] cursor-pointer hover:border-[#CC2200] transition-colors"
               id="label-{{ $uid }}">
          <i class="ti ti-upload text-2xl text-[#C0C0C0] mb-1" id="icon-{{ $uid }}"></i>
          <span class="text-[12px] text-[#94A3B8]" id="txt-{{ $uid }}">
            {{ ($row && $row->gorsel) ? 'Değiştir' : 'Görsel Seç' }}
          </span>
          <span class="text-[10px] text-[#C0C0C0] mt-0.5">JPG, PNG, WEBP · Maks. 5MB · 4000×4000px</span>
          <input type="file" name="f_{{ $uid }}" accept="image/*" class="hidden"
                 onchange="(function(inp){
                   const f=inp.files[0]; if(!f) return;
                   const lbl=document.getElementById('label-{{ $uid }}');
                   const ico=document.getElementById('icon-{{ $uid }}');
                   const txt=document.getElementById('txt-{{ $uid }}');
      ...
      @elseif($tip === 'html')
        ...
        @else
        <textarea name="f_{{ $alan }}_en" rows="10"
                  class="w-full px-4 py-2.5 border border-[#E2E8F0] rounded-[8px] text-[14px] focus:outline-none focus:border-[#CC2200] focus:ring-2 focus:ring-[rgba(204,34,0,0.08)] transition-colors resize-none leading-relaxed font-mono"
                  placeholder="HTML desteklenir…">{{ old("f_{$alan}_en", $row?->deger ?? '') }}</textarea>
        <p class="text-[11px] text-[#94A3B8] mt-1.5">HTML etiketleri desteklenir</p>
        @endif

      @elseif($tip === 'textarea')
        <textarea name="f_{{ $uid }}" rows="4"
                  ...
                  placeholder="{{ $alan_tanim['baslik'] }}">{{ old("f_{$uid}", $row?->deger ?? '') }}</textarea>

      @elseif($tip === 'url')
        <input type="url" name="f_{{ $uid }}"
               value="{{ old("f_{$uid}", $row?->deger ?? '') }}"
      ...
      @elseif($tip === 'bankaListesi')
        ...
        <div id="bankaContainer">
          ...
          <div class="banka-satir flex gap-3 items-start p-4 bg-[#F9F9F9] rounded-[10px] border border-[#E2E8F0] mb-3">
            ...
            <button type="button" class="banka-kaldir mt-6 w-8 h-8 flex items-center justify-center rounded-[6px] text-[#94A3B8] hover:bg-red-50 hover:text-red-500 transition-all shrink-0">
        ...
        <script>
        (function(){
          var c=document.getElementById('bankaContainer'),e=document.getElementById('bankaEkleBtn'),h=document.getElementById('bankaHidden');
          function sr(){var r=document.createElement('div');r.className='banka-satir flex gap-3 items-start p-4 bg-[#F9F9F9] rounded-[10px] border border-[#E2E8F0] mb-3';r.innerHTML='<div class="flex-1 grid grid-cols-3 gap-3"><div><label class="block text-[10px] font-medium text-[#64748B] uppercase tracking-[.06em] mb-1">Banka Adı</label><input type="text" data-field="banka_adi" placeholder="Ziraat Bankası" class="w-full px-3 py-2 border border-[#E2E8F0] rounded-[6px] text-[13px] focus:outline-none focus:border-[#CC2200] transition-colors"></div><div><label class="block text-[10px] font-medium text-[#64748B] uppercase tracking-[.06em] mb-1">IBAN</label><input type="text" data-field="iban" placeholder="TR00..." class="w-full px-3 py-2 border border-[#E2E8F0] rounded-[6px] text-[13px] font-mono focus:outline-none focus:border-[#CC2200] transition-colors"></div><div><label class="block text-[10px] font-medium text-[#64748B] uppercase tracking-[.06em] mb-1">Alıcı Adı</label><input type="text" data-field="alici_adi" placeholder="Ad Soyad" class="w-full px-3 py-2 border border-[#E2E8F0] rounded-[6px] text-[13px] focus:outline-none focus:border-[#CC2200] transition-colors"></div></div><button type="button" class="banka-kaldir mt-6 w-8 h-8 flex items-center justify-center rounded-[6px] text-[#94A3B8] hover:bg-red-50 hover:text-red-500 transition-all shrink-0"><i class="ti ti-trash text-sm"></i></button>';return r;}
          function s(){var rows=c.querySelectorAll('.banka-satir'),d=[];rows.forEach(function(r){d.push({banka_adi:r.querySelector('[data-field="banka_adi"]').value,iban:r.querySelector('[data-field="iban"]').value,alici_adi:r.querySelector('[data-field="alici_adi"]').value});});h.value=JSON.stringify(d);}
          function rb(){var rows=c.querySelectorAll('.banka-satir');rows.forEach(function(r){var b=r.querySelector('.banka-kaldir');if(rows.length===1){b.disabled=true;b.classList.add('opacity-30','cursor-not-allowed');}else{b.disabled=false;b.classList.remove('opacity-30','cursor-not-allowed');}});}
          e.addEventListener('click',function(){c.appendChild(sr());rb();s();});
          c.addEventListener('click',function(ev){var b=ev.target.closest('.banka-kaldir');if(!b)return;if(c.querySelectorAll('.banka-satir').length>1){b.closest('.banka-satir').remove();rb();s();}});
          c.addEventListener('input',s);
          document.querySelector('form').addEventListener('submit',s,true);
          rb();s();
        })();
        </script>
        @endpush
        @else
        <p class="text-[13px] text-[#94A3B8] italic">Banka listesi yalnızca Türkçe olarak yönetilir.</p>
        @endif

      @else
        {{-- metin --}}
        <input type="text" name="f_{{ $uid }}"
               value="{{ old("f_{$uid}", $row?->deger ?? '') }}"
...
<style>
  .ql-toolbar.ql-snow { border:none; border-bottom:1px solid rgba(0,0,0,.10); background:#F9F9F9; padding:8px 12px; }
  .ql-container.ql-snow { border:none; font-family:inherit; }
  .ql-editor { padding:14px 16px; min-height:260px; font-size:14px; line-height:1.65; color:#0F172A; }
...
<script>
function dilSec(dil) {
  document.getElementById('panel-tr').style.display = dil === 'tr' ? '' : 'none';
  document.getElementById('panel-en').style.display = dil === 'en' ? '' : 'none';
  var aktif   = 'border-[#CC2200] bg-[#CC2200] text-white';
  var pasif   = 'border-[#E2E8F0] bg-white text-[#64748B]';
  var sekmeTr = document.getElementById('sekme-tr');
  var sekmeEn = document.getElementById('sekme-en');
  if (dil === 'tr') {
    sekmeTr.className = sekmeTr.className.replace('border-[#E2E8F0] bg-white text-[#64748B]', 'border-[#CC2200] bg-[#CC2200] text-white');
    sekmeEn.className = sekmeEn.className.replace('border-[#CC2200] bg-[#CC2200] text-white', 'border-[#E2E8F0] bg-white text-[#64748B]');
  } else {
    sekmeEn.className = sekmeEn.className.replace('border-[#E2E8F0] bg-white text-[#64748B]', 'border-[#CC2200] bg-[#CC2200] text-white');
    sekmeTr.className = sekmeTr.className.replace('border-[#CC2200] bg-[#CC2200] text-white', 'border-[#E2E8F0] bg-white text-[#64748B]');
  }
}
