...
@section('page-title', 'Katalog Oluşturucu')

@push('styles')
<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;600;700&display=swap" rel="stylesheet">
<style>
.katalog-builder{display:flex;height:calc(100vh - 71px);margin:-1.5rem;overflow:hidden;}
...
@media print{
    *,-webkit-*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}
    ...
    .katalog-builder{display:block!important;height:auto!important;overflow:visible!important;margin:0!important;}
    ...
    .katalog-sayfa{width:210mm!important;min-height:297mm!important;margin:0!important;box-shadow:none!important;border-radius:0!important;page-break-after:always;break-after:page;}
...
function katalogBuilder() {
    ...
    return {
        ...
        tocAdi: function(urun) {
            return this.icindekiler[urun.id] || urun.ad;
        },
        ...
        secilmisMi: function(id) {
            return this.seciliUrunler.some(function(u){ return u.id === id; });
        },
        ...
        urunleriYukle: function() {
            ...
            fetch('{{ route("admin.katalog.urunler") }}')
        ...
        ekstraEkle: function() {
            this.kapak.ekstralar.push({ etiket: '', deger: '' });
        },

        ekstraSil: function(i) {
            this.kapak.ekstralar.splice(i, 1);
        },
        ...
        kaydet: function() {
            ...
            fetch('{{ route("admin.katalog.kaydet") }}', {
                method: 'POST',
                headers: {
        ...
        logoYukle: function(event) {
            ...
            var fd = new FormData();
            fd.append('logo', file);
            fetch('{{ route("admin.katalog.logo-yukle") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Accept': 'application/json',
        ...
        katalogYazdir: function(id) {
            window.open('{{ url("admin/katalog") }}/' + id + '/yazdir', '_blank');
        },

        katalogYukle: function(k) {
            window.location.href = '{{ route("admin.katalog.index") }}?katalog=' + k.id;
        },

        katalogSil: function(id) {
            var self = this;
            if (!confirm('Bu kataloğu silmek istediğinizden emin misiniz?')) return;
            fetch('{{ url("admin/katalog") }}/' + id, {
                method: 'DELETE',
...
<div x-data="katalogBuilder()" class="katalog-builder">
  ...
  <aside class="kb-side">
    ...
    <div x-show="basariMesaji" class="kb-toast-ok">
      <i class="ti ti-circle-check"></i><span x-text="basariMesaji"></span>
    </div>
    <div x-show="hataMesaji" class="kb-toast-err">
      ...
      <div style="border-top:1px solid rgba(255,255,255,0.1);padding-top:14px;margin-top:4px;">
        ...
        <div style="border-top:1px solid rgba(255,255,255,0.08);padding-top:12px;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
            ...
            <button @click="ekstraEkle()"
                    style="display:flex;align-items:center;gap:4px;font-size:10px;color:#B8962E;background:transparent;border:none;cursor:pointer;font-family:inherit;padding:0;">
              <i class="ti ti-plus" style="font-size:11px;"></i> Ekle
            </button>
          </div>
          <div x-show="kapak.ekstralar.length===0"
               style="font-size:11px;color:rgba(255,255,255,0.2);font-style:italic;padding:2px 0 8px;">
            Avans oranı, teklif süresi gibi ek bilgiler…
          </div>
          <template x-for="(ekstra, ei) in kapak.ekstralar" :key="ei">
            <div class="kb-ekstra-row">
              ...
              <button @click="ekstraSil(ei)" class="kb-ctrl del" style="flex-shrink:0;">
                <i class="ti ti-x"></i>
              </button>
            </div>
          </template>
    ...
    <div x-show="aktifTab==='urunler'">
      ...
      <div class="kb-scroll" style="max-height:270px;border-bottom:1px solid rgba(255,255,255,0.1);">
        ...
        <div x-show="!yukleniyorUrunler && filtreliUrunler.length===0"
      ...
      <div class="kb-scroll" style="max-height:200px;">
        <div x-show="seciliUrunler.length===0" style="padding:8px 12px;font-size:11px;color:rgba(255,255,255,0.25);font-style:italic;">
          ...
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
            <div style="min-width:0;flex:1;">
              ...
              <div style="font-size:10px;color:rgba(255,255,255,0.3);margin-top:2px;display:flex;gap:6px;">
                <span x-text="k.urun_sayisi + ' ürün'"></span>
                <span>·</span>
  ...
  <div class="katalog-preview">
    ...
    <div class="katalog-sayfa">
      ...
      <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;text-align:center;padding:20mm 6mm 8mm;">
        ...
        <div x-show="kapak.ihale_no || kapak.ihale_tarih || kapak.ihale_yeri || kapak.ihale_saat || kapak.ekstralar.length>0"
          ...
          <div x-show="kapak.ihale_no"
               style="display:flex;align-items:center;padding:12px 16px;border-top:1px solid #F1F5F9;">
          ...
          <template x-for="(ekstra, ei) in kapak.ekstralar" :key="'ek-'+ei">
            <div x-show="ekstra.etiket || ekstra.deger"
                 style="display:flex;align-items:center;padding:12px 16px;border-top:1px solid #F1F5F9;">
    ...
    <div class="katalog-sayfa">
      ...
      <div style="flex:1;">
        <div x-show="seciliUrunler.length===0"
             style="padding:32px;text-align:center;color:#CBD5E1;font-size:12px;font-style:italic;">
          Ürün eklendikçe içindekiler otomatik dolacaktır
        </div>
        <template x-for="(urun, i) in seciliUrunler" :key="urun.id+'-toc'">
          <div class="kb-toc-entry" style="padding:9px 0;border-bottom:1px solid #F1F5F9;">
            <span x-text="(i+1)+'.'"
                  style="min-width:26px;font-family:'Cormorant Garamond',serif;font-size:14px;color:#B8962E;font-weight:600;flex-shrink:0;"></span>
            <span
      ...
      <div class="katalog-sayfa">
        ...
        <div style="display:flex;flex:1;gap:18px;align-items:flex-start;">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <div x-html="urun.aciklama || ''"
                 style="font-size:12px;color:#334155;line-height:1.85;"></div>
            <div x-show="!urun.aciklama"
                 style="font-size:11px;color:#CBD5E1;font-style:italic;">Açıklama girilmemiş</div>
          </div>
        </div>
